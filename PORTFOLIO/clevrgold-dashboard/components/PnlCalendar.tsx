...
import { buildNewsMap, getMonthSummary, type NewsEvent } from '@/lib/news-calendar';
            ...
            <div key={i} className={cn(
              ...
              <div className="flex items-center gap-2 mb-1.5">
                <span className={cn(
                  ...
                  ev.impact === 'VERY_HIGH' ? 'bg-red-400' : 'bg-orange-400'
                )} />
              ...
              <div className="text-[11px] text-slate-400 space-y-0.5 pl-4">
                ...
                <div>Impact: <span className={ev.impact === 'VERY_HIGH' ? 'text-red-400' : 'text-orange-400'}>{ev.impact.replace('_', ' ')}</span></div>
...
export default function PnlCalendar() {
  ...
  // News data for current view month (month is 0-indexed, lib uses 1-indexed)
  const newsMap = useMemo(() => buildNewsMap(year, month + 1), [year, month]);
  const summary = useMemo(() => getMonthSummary(year, month + 1), [year, month]);
                ...
                return (
                  <tr key={wi} className="border-b border-[#1e2a3a] last:border-b-0">
                    {week.map((day, di) => {
                      ...
                      const topImpact = newsEvents?.some(e => e.impact === 'VERY_HIGH') ? 'VERY_HIGH' : 'HIGH';
                        ...
                        <td key={di} className="p-0 border-r border-[#1e2a3a]">
                          ...
                          >
                            ...
                            <div className="flex items-center gap-0.5 mb-0.5 w-full justify-center">
                              <span className={cn(
                                'text-[10px] font-mono',
                                isToday ? 'text-[#eab308]' : hasNews ? (topImpact === 'VERY_HIGH' ? 'text-red-400/80' : 'text-orange-400/80') : 'text-slate-500'
                              )}>{day}</span>
